...
function showOnboarding() {
  ...
  el.onboardOverlay.setAttribute("aria-hidden", "false");
  updateOnboardStep();
}
function closeOnboarding() {
  el.onboardOverlay.classList.remove("open");
  el.onboardOverlay.setAttribute("aria-hidden", "true");
  try { localStorage.setItem(ONBOARD_KEY, "1"); } catch {}
}
function updateOnboardStep() {
  document.querySelectorAll(".onboard-step").forEach((s, i) => s.classList.toggle("active", i === _onboardStep));
  document.querySelectorAll(".odot").forEach((d, i) => d.classList.toggle("active", i === _onboardStep));
  el.onboardNext.textContent = _onboardStep === ONBOARD_STEPS - 1 ? "Get started →" : "Next →";
}
el.onboardNext?.addEventListener("click", () => {
  if (_onboardStep < ONBOARD_STEPS - 1) { _onboardStep++; updateOnboardStep(); }
  else closeOnboarding();
});
el.onboardSkip?.addEventListener("click", closeOnboarding);
el.onboardOverlay?.addEventListener("click", (e) => { if (e.target === el.onboardOverlay) closeOnboarding(); });
document.querySelectorAll(".odot").forEach((d) => {
  d.addEventListener("click", () => { _onboardStep = Number(d.dataset.step); updateOnboardStep(); });
});
